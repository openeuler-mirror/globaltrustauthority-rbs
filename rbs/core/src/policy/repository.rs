...
pub trait PolicyRepository: Send + Sync {
    ...
    async fn find_by_name_and_user(&self, name: &str, username: &str) -> Result<Option<PolicyEntity>, PolicyError>;
...
impl PolicyRepository for SeaOrmPolicyRepository {
    async fn insert(&self, entity: &PolicyEntity) -> Result<(), PolicyError> {
        let model = entity::ActiveModel {
            ...
            policy_name: sea_orm::Set(entity.policy_name.clone()),
            policy_version: sea_orm::Set(entity.policy_version),
            policy_content: sea_orm::Set(entity.policy_content.clone()),
    ...
    async fn find_by_id(&self, policy_id: &str) -> Result<Option<PolicyEntity>, PolicyError> {
        ...
        Ok(model.map(|m| PolicyEntity {
            policy_id: m.policy_id, username: m.username, policy_name: m.policy_name,
