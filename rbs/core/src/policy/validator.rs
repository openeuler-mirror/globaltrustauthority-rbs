...
impl PolicyValidator {
    ...
    pub fn validate_name(&self, name: &str) -> Result<(), PolicyError> {
        if name.is_empty() || name.len() > self.config.max_name_len {
            log::error!("Policy name validation failed: length out of range (1..{}), got {}", self.config.max_name_len, name.len());
        ...
        if let Some(c) = name.chars().find(|c| self.config.name_blacklist.contains(c)) {
            log::error!("Policy name validation failed: name contains forbidden character '{}'", c);
    ...
    pub fn check_user_policy_count(&self, current_count: usize) -> Result<(), PolicyError> {
        if current_count >= self.config.max_per_user {
            log::error!("Policy count check failed: current {} exceeds max {}", current_count, self.config.max_per_user);
    ...
    pub fn decode_and_check_size(&self, content_type: &str, content: &str) -> Result<String, PolicyError> {
        if content_type != "base64" {
            log::error!("Policy content validation failed: unsupported content type '{}'", content_type);
            return Err(PolicyError::UnsupportedContentType { content_type: content_type.to_string() });
        }
        if content.is_empty() {
            log::error!("Policy content validation failed: content is empty");
        ...
        let decoded = base64::engine::general_purpose::STANDARD
            .decode(content)
            .map_err(|e| {
                log::error!("Policy content validation failed: base64 decode error: {}", e);
                PolicyError::ContentDecodeError { reason: format!("base64 decode failed: {}", e) }
            })?;

        let size_kb = decoded.len() / 1024;
        if size_kb > self.config.max_content_size_kb {
            log::error!("Policy content validation failed: size {}KB exceeds max {}KB", size_kb, self.config.max_content_size_kb);
            return Err(PolicyError::ContentTooLarge { size_kb, max_kb: self.config.max_content_size_kb });
        }

        String::from_utf8(decoded).map_err(|e| {
            log::error!("Policy content validation failed: decoded content is not valid UTF-8: {}", e);
